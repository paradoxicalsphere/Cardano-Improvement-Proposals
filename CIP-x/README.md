...
<blockquote>The increasing centralization of the network by a handful of entities is eroding the value of the ecosystem, and it might never come back...
...
Nevertheless... there’s naturally the incentive [for stake pool operators] to open more than one [pool]... the problem is that this becomes a positive feedback loop: you’re able to afford more marketing... and you’re able to fill more pools to saturation, which gets you even more resources and thus you can expand your operations... the success of a large multi-pool stake pool necessarily means that someone is worse off. There will only ever be 45 billion ADA, so this hard cap means that every time a delegator decides to go with an SPO, every other SPO on the network loses potential revenue (assuming that they don’t already have a saturated pool and don’t have any intentions of opening a new one).
