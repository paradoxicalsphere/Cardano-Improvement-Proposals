...
<a name="dependency"></a><sup>1</sup> The current CIP draft is based on the research of the [Center for Humane Technology](https://www.humanetech.com/). Decision-making processes having a requirement for an end goal of broad consensus—such as [CIP-1694](https://github.com/JaredCorduan/CIPs/blob/voltaire-v1/CIP-1694/README.md)—are out of scope for the current CIP. The current CIP proposes an approach to mitigate risks associated with token-weighted governance models, in which users with the most tokens have the biggest share of voting power.
