...
To implement the current CIP, each employee is assigned a unique closed set of wallet addresses. The private signing key file for a wallet address belonging to a closed set is accessible only to the employee assigned the wallet address and the employer.<sup><a href="#secret">3</a></sup>
